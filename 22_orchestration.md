...
2. **リトライは Workflows の `retry` で定義**する。既定は「指数バックオフ・最大3回・初回10秒」。
   ...
   * **共通サブワークフローに一律のリトライを置かない。** BigQuery実行のような共通サブワークフローは、
     冪等な参照（件数チェック・`MERGE`）と非冪等な書き込み（実行履歴への `INSERT`）の
     両方から呼ばれる。サブワークフロー側で一括してリトライを設定すると、
     非冪等な処理まで再実行される。**リトライは呼び出し側のステップで指定する。**
...
10. **Terraformの `templatefile()` で読む場合、Workflows自身の式は `$${...}` とエスケープする**。忘れるとplan時にエラーになるか、意図しない空文字が埋め込まれる。
    * **コメント行も展開の対象**である。解説のつもりでコメントに書いた `${...}` も
      Terraformが式として解釈しようとし、plan時にエラーになる。
      コメントに書く場合もエスケープするか、記号を使わず日本語で説明する。
11. **BigQueryを呼ぶステップは、戻り値の `jobComplete` を必ず検査する**。
    `jobs.query` はリクエストのタイムアウト内にジョブが終わらないと、
    **エラーではなく `jobComplete: false` を返して正常終了する**。
    素通りさせると「ワークフローは成功したのにデータが入っていない」という、
    最も追いにくい障害になる。`false` は例外として送出する。
    * 実行時間が読めないジョブは `jobs.insert` + `jobs.get` のポーリングにする（ルール6）。
    * 併せて `timeoutMs`（リクエスト側）とコネクタの `timeout` の両方を明示する（ルール3）。
