...
6. **マネージドサービスが認証情報を自身の設定値として保持する場合の例外。**
   BigQuery Data Transfer Service のコネクタ認証情報のように、
   サービス側がSecret Managerを参照する仕組みを持たず、
   認証情報を**自身の設定パラメータとして保持する**ものがある。
   この場合、`22` 章5節3（転送設定はTerraformで管理する）と本節2（値をtfstateに入れない）が
   **両立しない**。次の条件をすべて満たす場合に限り、tfstateへの記録を許容し、**ADRに記録する**。
   * 値の**正はSecret Managerに置く**。Terraformは `data` ソースで読み出すだけで、値を投入しない。
     投入は `*.secret.tfvars`（6節）を入力とするスクリプト、または対話入力で行う。
   * **stateバケットを機密として扱う**。`uniform_bucket_level_access` と
     `public_access_prevention` を有効にし、閲覧権限の付与をPRレビュー対象にする。
   * 該当パラメータを `lifecycle.ignore_changes` に入れる。
     サービス側が値をマスクして返すため、入れないと毎回差分が出続け、
     「差分が出ているのが常態」になってレビューが機能しなくなる。
   * ローテーション手順（Secret Manager更新 → 該当リソースの `-replace`）を設計書に書く。

## 6. コード・リポジトリ上の禁止事項

* 認証情報・トークン・実データのコミット。CIでシークレットスキャン（`gitleaks` 等）を必須にする。
* `*.tfvars` のうち機密を含むものは `.gitignore` 対象とし、`*.secret.tfvars` の命名で明示する。
  * **`*.secret.tfvars.example`（値を `REPLACE_ME` にしたテンプレート）はコミットする。**
    どのキーが必要か・値をどこから取得するかが分からないと、環境を再構築できない。
  * `.gitignore` には `*.secret.tfvars` と書く（`.example` は除外されない）。
    追加後に `git check-ignore -v` で実ファイルが除外され、テンプレートが除外されないことを確認する。
  * 実ファイルは `chmod 600`。平文を手元に置きたくない場合は、投入スクリプトを対話入力で使う。
